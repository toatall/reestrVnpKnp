...
<?php
    ...
    $this->widget('bootstrap.widgets.TbGridView',array(
    	...
    	//'filter'=>$model,
        //'rowCssClassExpression' => '($data["id"]%2 > 0) ? "" : "style_row"',
    	...
    	'columns'=>array(
    		...
    		// Заключение представленное в отдел урегулирования задолженности по наличию оснований для применения подп. 2 пун. 2 ст. 45 НК РФ
    		array(
    			'name'=>'query_in_otdel_UZ_conclusion',
    			'value'=>'($data->query_in_otdel_UZ_conclusion===0 ? "нет" : ($data->query_in_otdel_UZ_conclusion===null ? "" : "да"))',
    			'visible'=>in_array('query_in_otdel_UZ_conclusion', Yii::app()->session['viewColumnGridView']),
    		),
    		// Дата запроса заключения отделом урегулирования задолженности по установлению наличия оснований применения подп. 2 пун. 2 ст. 45 НК РФ
    		array(
    			'name'=>'query_in_otdel_UZ_date',
    			'visible'=>in_array('query_in_otdel_UZ_date', Yii::app()->session['viewColumnGridView']),
    		),
   			// Дата заключения представленного профильными  отделами инспекции в  отдел урегулирования задолженности
   			array(
				'name'=>'query_in_otdel_UZ_from_IFNS_date',
    			'visible'=>in_array('query_in_otdel_UZ_from_IFNS_date', Yii::app()->session['viewColumnGridView']),
    		),
    		...
    		array(
                ...
                'buttons'=>array(
                    'view'=>array(
                        /*'url'=>'Yii::app()->createUrl("reestrCheck/view", array("id"=>$data->id,"fast"=>true))',
                        'click'=>'function(){
                            id = $(this).attr("href");
                            viewCheckWindow(id);                                             
                            return false;
                        }',*/
                        'url'=>'Yii::app()->createUrl("reestrCheck/view", array("id"=>$data->id))',             
                    ),
                    'update'=>array(
                        /*'url'=>'Yii::app()->createUrl("reestrCheck/update", array("id"=>$data->id,"fast"=>true))',
                        'click'=>'function(){
                            id = $(this).attr("href");
                            viewCheckWindow(id);                                             
                            return false;
                        }',*/
                        'url'=>'Yii::app()->createUrl("reestrCheck/update", array("id"=>$data->id))',             
                    ),
                    'delete'=>array(
                        'label'=>'Перенос в архив',
                        'click'=>"''",
                        'url'=>'Yii::app()->createUrl("reestrCheck/delete", array("id"=>$data->id))',
                    ),                    
                ),
    		),
    	),
...
<script type="text/javascript">
    ...
    {
        var params = "fullscreen=yes,location=no,center=yes,modal=yes,menubar=no,scrollbars=yes,status=no,resizable=yes,toolbar=no,width=1000,height=800,top=10,left=10";
